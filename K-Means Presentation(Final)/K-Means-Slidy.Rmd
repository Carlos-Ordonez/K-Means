...
##k means assignment{.smaller}
\
\
\
\

$$Distance = ||D_{n,k}|| = \sqrt{\sum_i^n(D_{n.k})^2} = \sqrt{(x_{n,1}-\mu_{k,1})^2+\dots+(x_{n,d}-\mu_{k,d})^2}$$

##Loss Function{.build .bigger}

$$Distance_{Global} = \sqrt{\sum_{k=1}^k \sum_{n:x_n \in S_k} \sum_{d=1}^D (x_{n,d} - \mu_{k,d})^2} $$

$\sum_{k=1}^k$ = Sum amongst all clusters

$\sum_{n:x_n \in S_k}$ = for each point in the cluster

$\sum_{d=1}^D (x_{n,d} - \mu_{k,d})^2$ = the sum of the euclidean distance of each points feature to each respective centroid feature
...
1.Assign each datapoint to the cluster with the clostset centroid

2.Assign each centroid to be the mean of its clusters datapoints.
}
...
Repeat untill $S_1...S_k$ dont change{
  ...
  -find $\mu_k$ for $x_n$ with the smallest $Distance(\mu_k,x_n)$
...
Repeat untill $S_1...S_k$ dont change{
  ...
  $$\mu_k\leftarrow|S_k|^{-1} = \frac{1}{N}\sum_{n:x_n \in S_k} x_n$$
}
...
```{r out.width = "100%", out.height = "100%", fig.align="center", fig.cap="A3 Dataset Global Similarity for k=1-100", echo=FALSE}
